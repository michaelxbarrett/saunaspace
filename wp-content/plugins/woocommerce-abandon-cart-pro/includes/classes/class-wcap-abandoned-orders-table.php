...
class WCAP_Abandoned_Orders_Table extends WP_List_Table {
	...
	public function column_email( $abandoned_row_info ) {	
	    ...
	    if( isset( $abandoned_row_info->email ) ) {      
    	    ...
    	    $row_actions['edit']   = '<a href="' . wp_nonce_url( add_query_arg( array( 'action' => 'orderdetails', 'id' => $abandoned_order_id ), $this->base_url ), 'abandoned_order_nonce') . '">' . __( 'View order', 'woocommerce-ac' ) . '</a>';
	    ...
	    switch ( $column_name ) {
	        case 'id' :
			    if( isset( $wcap_abandoned_orders->id ) ) {
			     $value = '<strong><a href="admin.php?page=woocommerce_ac_page&action=orderdetails&id='.$wcap_abandoned_orders->id.' ">'.$wcap_abandoned_orders->id.'</a> </strong>';
			    }
				break;
			case 'customer' :
			    if( isset( $wcap_abandoned_orders->customer ) ) {
				    $value = $wcap_abandoned_orders->customer;
			    }
				break;
			case 'order_total' :

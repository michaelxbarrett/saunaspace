...
$edd_updater = new EDD_AC_WOO_Plugin_Updater( EDD_SL_STORE_URL_AC_WOO, __FILE__, array(
        'version'   => '3.6',                     // current version number
    	...
    	************************************************/
    	function wcap_update_db_check() {
        	...
        	if ( $woocommerce_ac_plugin_version != $this->wcap_get_version() ) {
        		...
        		update_option( 'woocommerce_ac_db_version', '3.6' );
        	...
        	$table_name = $wpdb->prefix . "ac_email_templates";
        	$check_template_table_query = "SHOW COLUMNS FROM $table_name LIKE 'wc_email_header' ";
        	$results = $wpdb->get_results( $check_template_table_query );
        	 
        	if ( count( $results ) == 0 ) {
        	    $alter_template_table_query = "ALTER TABLE $table_name
        	    ADD COLUMN `wc_email_header` varchar(50) COLLATE utf8_unicode_ci NOT NULL AFTER `is_wc_template`";
        	    $wpdb->get_results( $alter_template_table_query );
        	}
        	
        	$table_name = $wpdb->prefix . "ac_abandoned_cart_history";
        	$check_abandoned_cart_history_table_query = "SHOW COLUMNS FROM $table_name LIKE 'language' ";
        	$results_abandoned_cart_history = $wpdb->get_results( $check_abandoned_cart_history_table_query );
        	 
        	if ( count( $results_abandoned_cart_history ) == 0 ) {
        	    $alter_abandoned_cart_history_table_query = "ALTER TABLE $table_name
        	    ADD COLUMN `language` varchar(50) COLLATE utf8_unicode_ci NOT NULL AFTER `user_type`";
        	    $wpdb->get_results( $alter_abandoned_cart_history_table_query );
        	}
        	
        	$table_name = $wpdb->prefix . "ac_abandoned_cart_history";
        	$check_abandoned_cart_history_table_query = "SHOW COLUMNS FROM $table_name LIKE 'session_id' ";
        	$results_abandoned_cart_history = $wpdb->get_results( $check_abandoned_cart_history_table_query );
        	
        	if ( count( $results_abandoned_cart_history ) == 0 ) {
        	    $alter_abandoned_cart_history_table_query = "ALTER TABLE $table_name
        	    ADD COLUMN `session_id` varchar(50) COLLATE utf8_unicode_ci NOT NULL AFTER `language`";
        	    $wpdb->get_results( $alter_abandoned_cart_history_table_query );
        	}
                    ...
                    if ( count( $results ) == 0 ) {
                        ...
                        if ( $blank_cart_info != $cart_info ){
                            $insert_query = "INSERT INTO `" . $wpdb->prefix . "ac_abandoned_cart_history`
                                            ( user_id , abandoned_cart_info , abandoned_cart_time , cart_ignored , recovered_cart, user_type, language )
                                            VALUES ( '" . $user_id."' , '" . $cart_info."' , '" . $current_time . "' , '0' , '0' , 'REGISTERED', '". $current_user_lang ."' )";
                        ...
                        if ( ($results[0]->language == $current_user_lang ||  $results[0]->language == '') && $blank_cart_info != $updated_cart_info ){
                            ...
                            if ( ! $this->wcap_compare_carts( $user_id, $results[0]->abandoned_cart_info ) ) {
                                ...
                                $query_update = "INSERT INTO `" . $wpdb->prefix . "ac_abandoned_cart_history`
                                                ( user_id, abandoned_cart_info, abandoned_cart_time, cart_ignored, recovered_cart, user_type, language  )
                                                VALUES ( '" . $user_id . "', '" . $updated_cart_info . "', '" . $current_time . "', '0', '0', 'REGISTERED', '". $current_user_lang ."' )";
                        ...
                        if ( ( $results[0]->language == $current_user_lang ||  $results[0]->language == '' )  && $blank_cart_info != $updated_cart_info ){
                            ...
                            if ( ! $this->wcap_compare_carts( $user_id, $results[0]->abandoned_cart_info ) ) {
                                $query_update = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET abandoned_cart_info = '" . $updated_cart_info . "', abandoned_cart_time  = '" . $current_time . "', language = '". $current_user_lang ."'  WHERE user_id ='" . $user_id . "' AND cart_ignored='0' ";
                                $wpdb->query( $query_update );
                ...
                if ( count($results) > 0 ) {
                    ...
                    if ( $guest_cart != "on" ) {
                        ...
                        if ( $updated_cart_info != $guest_blank_cart_info && ! $this->wcap_compare_only_guest_carts( $updated_cart_info, $results[0]->abandoned_cart_info ) ) {
                                ...
                                $query_update = "INSERT INTO `" . $wpdb->prefix."ac_abandoned_cart_history` ( user_id, abandoned_cart_info, abandoned_cart_time, cart_ignored, recovered_cart, user_type, language )
                                                VALUES ( '" . $user_id."', '" . $updated_cart_info . "', '".$current_time . "', '0', '0', 'GUEST', '". $current_user_lang ."' )";
                                $wpdb->query( $query_update ); 
                            ...
                            if ( $updated_cart_info!= $guest_blank_cart_info ) {
                                $query_update = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET abandoned_cart_info = '" . $updated_cart_info . "', abandoned_cart_time = '" . $current_time . "' WHERE user_id='" . $user_id . "' AND cart_ignored='0' ";
                                $wpdb->query( $query_update );
                            }
                        }
                    }
                    ...
                    if ( $track_guest_user_cart_from_cart == "on" &&  $get_cookie[0] != ''  ){
                        ...
                        if ( count( $results ) == 0 ) {
                            ...
                            if ( $blank_cart_info != $cart_info ){
                                $insert_query = "INSERT INTO `" . $wpdb->prefix . "ac_abandoned_cart_history`
                                                ( abandoned_cart_info , abandoned_cart_time , cart_ignored , recovered_cart, user_type, language, session_id  )
                                                VALUES ( '" . $cart_info."' , '" . $current_time . "' , '0' , '0' , 'GUEST', '". $current_user_lang ."', '". $get_cookie[0] ."' )";
                                $wpdb->query( $insert_query );
                            }
                            ...
                            if ( ($results[0]->language == $current_user_lang ||  $results[0]->language == '') && $blank_cart_info != $updated_cart_info ){
                                ...
                                if ( ! $this->wcap_compare_only_guest_carts( $updated_cart_info, $results[0]->abandoned_cart_info ) ) {
                                    ...
                                    $query_ignored = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET cart_ignored = '1' WHERE session_id ='" . $get_cookie[0] . "'";
                                    $wpdb->query( $query_ignored );
                        
                                    $query_update = "INSERT INTO `" . $wpdb->prefix . "ac_abandoned_cart_history`
                                                    ( abandoned_cart_info, abandoned_cart_time, cart_ignored, recovered_cart, user_type, language, session_id  )
                                                    VALUES ( '" . $updated_cart_info . "', '" . $current_time . "', '0', '0', 'GUEST', '". $current_user_lang ."', '". $get_cookie[0] ."' )";
                                    $wpdb->query( $query_update );
                                }
                            }
                        } else {
                            ...
                            if ( ( $results[0]->language == $current_user_lang ||  $results[0]->language == '' )  && $blank_cart_info != $updated_cart_info ){
                                ...
                                if ( ! $this->wcap_compare_only_guest_carts( $updated_cart_info, $results[0]->abandoned_cart_info ) ) {
                                    $query_update = "UPDATE `" . $wpdb->prefix . "ac_abandoned_cart_history` SET abandoned_cart_info = '" . $updated_cart_info . "', abandoned_cart_time  = '" . $current_time . "', language = '". $current_user_lang ."'  WHERE session_id ='" . $get_cookie[0] . "' AND cart_ignored='0' ";
                                    $wpdb->query( $query_update );
                                }
                            }
                        }

...
class Wcap_Abandoned_Cart_Cron_Job_Class {		   
     ...
     */
    public function wcap_abandoned_cart_send_email_notification() {
        global $wpdb;
        global $woocommerce;
            ...
            if ( $results_query_email ) {
                ...
                for ( $i = 0; $i < count( $results_query_email ); $i++ ) {
                    ...
                    if ( $order_date == $today_date ) {
                        $query_delete = "DELETE FROM `" . $wpdb->prefix . "ac_abandoned_cart_history` WHERE user_id = '" . $value->id . "'";
                        $wpdb->query( $query_delete );
                        $query_guest = "DELETE FROM `" . $wpdb->prefix . "ac_guest_abandoned_cart_history` WHERE email_id = '" . $value->email_id . "'";
        ...
        foreach ( $results_list as $key => $value ) {            
            ...
            if ( is_array( $results_query_email ) && count( $results_query_email ) > 0 ) {                    
                for ( $i = 0; $i < count( $results_query_email ); $i++ ) {
                    ...
                    if ( $order_date == $today_date ) {
                        $query_delete = "DELETE FROM `" . $wpdb->prefix . "ac_abandoned_cart_history`
        ...
        if ( $enable_email == 'on' ) {
            ...
            foreach ( $results_template as $results_template_key => $results_template_value ) {                
                ...
                foreach ( $carts as $key => $value ) {                        
                    ...
                    if( count( $cart ) > 0 && $value->user_id != '0' ) {                    
    ...
    ******/
    public function encrypt_validate( $validate ) {
     ...
     */
    public function get_carts( $template_to_send_after_time, $cart_abandon_cut_off_time ) {
        global $wpdb;		
    ...
    public function get_delete_carts( $template_to_send_after_time, $cart_abandon_cut_off_time ) {    
    	global $wpdb;
    	$cart_time = current_time( 'timestamp' ) - $template_to_send_after_time - $cart_abandon_cut_off_time;
    	if ( is_multisite() ){
    	    // get main site's table prefix
    ...
    public function check_sent_history( $user_id, $cart_update_time, $template_id, $id ) {
        global $wpdb;
    ...
    public function wp_coupon_code( $discount_amt, $get_discount_type, $get_expiry_date, $coupon_post_meta ) {        
    ...
    public function wp_random_string() {
    ...
    public function wcap_get_translated_texts( $get_translated_text, $message, $language ) {
        if( function_exists( 'icl_register_string' ) ) {
            if ( $language == 'en' ) {
                return $message;
            } else {

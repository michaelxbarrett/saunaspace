  ...
  **/
  ...
  function prepare_product($woo_product, $retailer_id = null) {
    ...
    if (wc_get_cart_url()) {
      ...
      $checkout_url = wc_get_cart_url() . $char;
      if (!filter_var($checkout_url , FILTER_VALIDATE_URL)) {
        $checkout_url = get_site_url() . $checkout_url ;
      }
    ...
    $price = floatval($woo_product->get_display_price()) * 100;
    ...
    foreach ($post_ids as $post_id) {
      ...
      $this->display_sticky_message(
        sprintf(
          ...
          ($total_old === $total) ? $count : min($count + $total_old, $total),
          $total),
        true);

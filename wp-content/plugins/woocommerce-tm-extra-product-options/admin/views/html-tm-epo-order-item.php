            ...
            if(empty($item_meta['tm_has_dpd']) && empty($item_meta['_tm_has_dpd'])){
                if ($epo['quantity'] <=0){
                    ...
                    ($epo_is_fee
                        ...
                        :wc_price( tc_order_get_price_excluding_tax( $order, $item_id, array('qty'=>1,'price'=>$epo['price']) ) / $epo['quantity'] )
                        )
        ...
        <div class="view">
            <?php 
                if(empty($item_meta['tm_has_dpd']) && empty($item_meta['_tm_has_dpd'])){
                    ...
                    ($epo_is_fee
                        ?wc_price(  (float) $epo['price'] )
                        :wc_price(  (float) tc_order_get_price_excluding_tax( $order, $item_id, array('qty'=>1,'price'=>$epo['price']) ) * (float) $item_meta['_qty'][0] )
                        )

...
final class TM_EPO_HELPER_base {
	...
	public function get_attachment_id( $attachment_url = '' ) {
		...
		$session_get = WC()->session->get('tc_attachment_ids');

		if (!is_array($session_get)){
			$session_get = array();
		}

		if (isset( $session_get[ $attachment_url ] )){
			return $session_get[ $attachment_url ];
		}
		...
		$session_get[ $attachment_url ] = $attachment_id;

		WC()->session->set('tc_attachment_ids', $session_get);

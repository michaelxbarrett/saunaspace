			...
			public function get_coupon_meta_data( $coupon ) {
				...
				switch ( $discount_type ) {
					...
					default:
						...
						$coupon_data['coupon_amount'] = apply_filters( 'wc_sc_coupon_amount', wc_price( $coupon_amount ), $coupon );
						break;
				...
				if ( ! empty( $post ) && isset( $post['generate_and_import'] ) ) {
					...
					for ( $i = 1; $i <= $post['no_of_coupons_to_generate']; $i++ ) {
						...
						if ( 'fixed_cart' == $post['discount_type'] ) {
							$data[ $i ]['discount_type'] = ( $this->is_wc_gte_30() ) ? 'Fixed cart discount' : 'Cart Discount';
						} elseif ( 'percent' == $post['discount_type'] ) {
							$data[ $i ]['discount_type'] = ( $this->is_wc_gte_30() ) ? 'Percentage discount' : 'Cart % Discount';
						} elseif ( 'fixed_product' == $post['discount_type'] ) {
							$data[ $i ]['discount_type'] = ( $this->is_wc_gte_30() ) ? 'Fixed product discount' : 'Product Discount';

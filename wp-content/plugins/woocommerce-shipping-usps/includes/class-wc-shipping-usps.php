...
class WC_Shipping_USPS extends WC_Shipping_Method {
						...
						foreach ( $usps_packages as $usps_package ) {
							...
							foreach ( $this->services as $service => $values ) {
								...
								if ( $rate_cost ) {
									...
									if ( 'per_item' === $this->packing_method ) {
										$rate_package['qty'] = $cart_item_qty;
									}

									$this->prepare_rate( $rate_code, $rate_id, $rate_name, $rate_cost, $rate_package );
								}
							}
						}
					} else {
	 ...
	 * @param array $rate_package Rate package.
	 * @return void
	 */
	private function prepare_rate( $rate_code, $rate_id, $rate_name, $rate_cost, $rate_package ) {
		// Name adjustment.
		...
		if ( ! empty( $rate_package ) ) {
			// Turn the box information into human readable form.
			$box_information = $rate_package['length'] . 'x' . $rate_package['width'] . 'x' . $rate_package['height'] . '(in) ' . $rate_package['weight'] . 'lbs';

			// Add quantity if shipping per item and there is more than one of that item in the cart.
			if ( 'per_item' === $this->packing_method && $rate_package['qty'] > 1 ) {
				$box_information .= ' x ' . $rate_package['qty'];
			}

			if ( isset( $this->found_rates[ $rate_id ] ) && array_key_exists( 'meta_data', $this->found_rates[ $rate_id ] ) ) {
				$meta_data = $this->found_rates[ $rate_id ]['meta_data'];
				$meta_data['Package ' . $packages ] = $box_information;
			} else {
				$meta_data = array( 'Package ' . $packages => $box_information );
			}
		}

		// Weight based shipping doesn't have package information.
		if ( 'weight_based' === $this->packing_method ) {
			$meta_data = array( 'Packing' => 'Weight Based Shipping' );
		}
